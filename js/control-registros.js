...
btnFiltroRegistroAdmin.addEventListener("click", function (e) {
    ...
    if (!usuarioSeleccionado) {
        alert("Por favor, seleccione un usuario para ver su historial.");
        return;
    }
    ...
    if (inputFechaDeInicio.value) {
        //reemplazo el guion, porque no funciona correctamente(tiene que ser con barras siempre). 
        console.log(inputFechaDeInicio.value)
    ...
    if (partesFecha.length === 3) {
        fechaCarrito = new Date(partesFecha[2], partesFecha[1] - 1, partesFecha[0]);
    } else {
    ...
    crearDomElements.createRegistroCompra(carrito, contenedorCompras, i);
    comprasMostradas++;
}
});
